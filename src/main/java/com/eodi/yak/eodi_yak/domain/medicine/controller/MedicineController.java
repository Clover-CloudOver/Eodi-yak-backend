...
import com.eodi.yak.eodi_yak.domain.medicine.GrpcPharmacyClient;
import com.eodi.yak.eodi_yak.domain.medicine.entity.Medicine;
import com.eodi.yak.eodi_yak.domain.medicine.entity.Pharmacy;
...
public class MedicineController {
    ...
    private final GrpcPharmacyClient grpcPharmacyClient;
    ...
    public List<MedicineResponse> searchMedicine(@RequestParam(defaultValue="타이레놀") String name,
        ...
        // TODO: 하드코딩된 반경 수정
        Integer radius = 100;
        PharmacyOuterClass.Pageable pageable = PharmacyOuterClass.Pageable.newBuilder()
                .setPage(page)
                .setSize(size)
                .build();

        PharmacyOuterClass.PharmacyNearRequest pharmacyNearGrpcRequest = PharmacyOuterClass.PharmacyNearRequest.newBuilder()
        ...
        PharmacyOuterClass.PageResponse nearbyPharmacies = grpcPharmacyClient.findNearbyPharmacies(pharmacyNearGrpcRequest);

        System.out.println("약 이름: " + name);
        // 반경 내 약국들의 paCode 리스트 추출

        List<String> pharmacyCodes = new ArrayList<>();
        for (PharmacyOuterClass.Pharmacy pharmacy : nearbyPharmacies.getResultsList()) {
            System.out.println("반경 내 약국의 pacode: " + pharmacy.getPaCode());
            pharmacyCodes.add(pharmacy.getPaCode());
        }
        ...
        for (PharmacyOuterClass.Pharmacy pharmacy : nearbyPharmacies.getResultsList()) {
            ...
            for (Medicine medicine : medicines) {
                if (medicine.getPharmacy().getPaCode().equals(pharmacy.getPaCode()) && medicine.getId().getMeName().equals(name)) {
                    response.add(MedicineResponse.from(medicine));
                    isMedicineFound = true;
                }
            }

            // 해당 약국에 재고가 없는 경우, 한 번만 재고 0으로 추가
            if (!isMedicineFound) {
                // 재고 0으로 Medicine 객체 생성 후 추가
                Pharmacy pharmacyEntity = new Pharmacy(pharmacy.getPaCode(), pharmacy.getPaName(),
                        new BigDecimal(pharmacy.getLatitude()), new BigDecimal(pharmacy.getLongitude()), pharmacy.getPhoneNumber(),
